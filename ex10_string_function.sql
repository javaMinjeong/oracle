...
SELECT
	first_name
FROM employees
	WHERE first_name LIKE '%an%' OR first_name LIKE '%An%' OR first_name LIKE '%An%' OR first_name LIKE '%aN%';	--Anna
...
SELECT * FROM tblinsa WHERE substr(name, 1, 1) = '김';	--12명
...
--조건절에서 사용
SELECT name, length(name) FROM tblcountry WHERE LENGTH(name) > 3;	--개인데이타 함수는 사용 가능

SELECT name, length(name) AS leng FROM tblcountry WHERE leng > 3;	--ORA-00904: "LENG": invalid identifier 실행 순서 생각하기!!

SELECT name, length(name) AS leng FROM tblcountry ORDER BY leng asc; -- 실행 순서 생각하기!!
...
/*
	...
	- 검색어의 위치 반환
	- number instr(칼럼, 검색어)
	- number instr(칼럼, 검색어, 시작위치)
	- number instr(칼럼, 검색어, 시작위치, -1)		//lastIndexOf
	- 못찾으면 0을 반환
...
SELECT
	lpad('a', 5),		--%5s
	lpad('a',5,'b'),	-- ㅇ자리를 b로 다 채우고 마지막 자리만 a로 채워라
	lpad('aa',5,'b'),	
	lpad('aaa',5,'b'),	
	lpad('aaaa',5,'b'),	
	lpad('aaaaa',5,'b'),	
...
SELECT
	...
	END AS g1,
	REPLACE(replace(gender, 'm', '남자'), 'f', '여자') AS g2,	-- 원본 반환
	decode(gender, 'm', '남자', 'f', '여자' ) AS g3	-- 값이 없을시 null반환
FROM tblcomedian;
